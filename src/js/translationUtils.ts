...
/**
 * * Helper to remove any locale prefix from a path
 * e.g. /en/about -> about, /about -> about, /en/en/about -> about
 */
  ...
  while (found) {
    ...
    for (const loc of locales) {
      if (cleaned === loc || cleaned.startsWith(loc + "/")) {
        cleaned = cleaned.substring(loc.length).replace(/^\/+/, "");
        found = true;
        break;
      }
    }
...
export function getLocalizedRoute(
  ...
  // Sanitize the route by removing any existing locale prefixes
  routeWithoutFragment = removeLocalePrefix(routeWithoutFragment);
...
export async function getLocalizedPathname(
  ...
  const routeStringTrimmed = removeLocalePrefix(oldPath);
